...
select @cleanup = 1
if (@cleanup = 1) begin
drop table if exists AllNeighborsSummaryDR
end

--1+5 minutes for CSBMS01
--27 minutes for CS2
--4 minutes for CS34
--16 minutes for CS1
--20 minutes for CS13
--2 minutes for CS36
if object_id('AllNeighborsSummaryDR') is null begin
	create table AllNeighborsSummaryDR (
		...
		CONSTRAINT PK_AllNeighborsSummaryDR PRIMARY KEY (distancebinselectionid, cellid)
	);
	...
	select dbtu.distancebin, a.sampleid, a.cellid, a.tdist / 2 tdist_microns, a.rdist / 2 rdist_microns, inlymphnode,
		...
		into #tmp
		from RandomCellMoreCols a cross join DistanceBinsToUse dbtu
		                          left join AllNeighborsHistDR ptype0 on a.cellid = ptype0.cellid and ptype0.ptype_lung = 0 and dbtu.distancebin = ptype0.distancebin
                                  left join AllNeighborsHistDR ptype1 on a.cellid = ptype1.cellid and ptype1.ptype_lung = 1 and dbtu.distancebin = ptype1.distancebin
                                  left join AllNeighborsHistDR ptype2 on a.cellid = ptype2.cellid and ptype2.ptype_lung = 2 and dbtu.distancebin = ptype2.distancebin
                                  left join AllNeighborsHistDR ptype3 on a.cellid = ptype3.cellid and ptype3.ptype_lung = 3 and dbtu.distancebin = ptype3.distancebin
                                  left join AllNeighborsHistDR ptype4 on a.cellid = ptype4.cellid and ptype4.ptype_lung = 4 and dbtu.distancebin = ptype4.distancebin
                                  left join AllNeighborsHistDR ptype5 on a.cellid = ptype5.cellid and ptype5.ptype_lung = 5 and dbtu.distancebin = ptype5.distancebin

	insert into AllNeighborsSummaryDR (
		distancebinselectionid, sampleid, cellid, tdist_microns, rdist_microns, inlymphnode,
    ...
    select distancebinselectionid, sampleid, cellid, tdist_microns, rdist_microns, inlymphnode,
           sum(count0) count0,
           sum(count1) count1,
           sum(count2) count2,
           sum(count3) count3,
	...
	group by distancebinselectionid, sampleid, cellid, tdist_microns, rdist_microns, inlymphnode
	create index i_sampleid on AllNeighborsSummaryDR(distancebinselectionid, sampleid)
