...
select @cleanup = 1
if (@cleanup = 1) begin
drop table if exists DiscriminantsTableDR_allcells
end

--these are with just lung training set
--~30 minutes for CS2
--5 for CSBMS01
--3 for CS34
--these are with lung and melanoma
--22 for CS13
--19 for CS1
--8 for CSBMS01
--2 for CS36
if object_id('DiscriminantsTableDR_allcells') is null begin
	create table DiscriminantsTableDR_allcells (
		...
		D5_raw real,
		D5 real,
		CONSTRAINT PK_DiscriminantsTableDR PRIMARY KEY (trainingsetid, distancebinselectionid, cellid)
	);
    ...
    with c as (
        select *, floor(tdist_microns / @divide) * @divide lower_edge from AllNeighborsSummaryDR
    ),	t as (
		select distinct trainingsetid, distancebinselectionid from TrainedModels
	), ProbabilitiesTable as (
		select c.sampleid, c.cellid, t.trainingsetid, d.distancebinselectionid, c.tdist_microns,
	...
	insert DiscriminantsTableDR_allcells (sampleid, cellid, tdist_microns, trainingsetid, distancebinselectionid, n0, n1, n2, n3, n4, n5, anyneighbors, anytrainingcells, D0_raw, D1_raw, D2_raw, D3_raw, D4_raw, D5_raw, D5)
	select sampleid, cellid, p.tdist_microns, p.trainingsetid, distancebinselectionid, n0, n1, n2, n3, n4, n5, anyneighbors, anytrainingcells,
	       case when anyneighbors+anytrainingcells=2 then n0*1./(n0+n1+n2+n3+n4+n5) else null end D0_raw,
		   case when anyneighbors+anytrainingcells=2 then n1*1./(n0+n1+n2+n3+n4+n5) else null end D1_raw,
		   case when anyneighbors+anytrainingcells=2 then n2*1./(n0+n1+n2+n3+n4+n5) else null end D2_raw,
		   case when anyneighbors+anytrainingcells=2 then n3*1./(n0+n1+n2+n3+n4+n5) else null end D3_raw,
		   case when anyneighbors+anytrainingcells=2 then n4*1./(n0+n1+n2+n3+n4+n5) else null end D4_raw,
		   case when anyneighbors+anytrainingcells=2 then n5*1./(n0+n1+n2+n3+n4+n5) else null end D5_raw,
		   case when anyneighbors+anytrainingcells=2 then n5*1./((n0+n1+n2+n3+n4)*cconstant+n5) else null end D5
		   from ProbabilitiesTable p
		   inner join cconstants cc on p.trainingsetid = cc.trainingsetid
	create index i_trainingsetid_sampleid_cellid on DiscriminantsTableDR_allcells(trainingsetid, sampleid, cellid)
	create index i_trainingsetid_cellid on DiscriminantsTableDR_allcells(trainingsetid, cellid)
	create index i_sampleid_anyneighbors_anytrainingcells on DiscriminantsTableDR_allcells(sampleid, anyneighbors, anytrainingcells)
	create index i_anyneighbors_anytrainingcells on DiscriminantsTableDR_allcells(anyneighbors, anytrainingcells) include (sampleid, D5)
end
go

create or alter view DiscriminantsTableDR as
select *
from DiscriminantsTableDR_allcells
where anyneighbors=1 and anytrainingcells=1
go

create or alter view DiscriminantsTableDR_morecols as
...
from DiscriminantsTableDR d
left join AllNeighborsSummaryDR a on d.cellid = a.cellid and d.distancebinselectionid = a.distancebinselectionid
go

create or alter view DiscriminantsTableDR_allcells_morecols as
select d.*, a.rdist_microns, a.count0, a.count1, a.count2, a.count3, a.count4, a.count5, a.count0PD1negPDL1neg, a.count0PD1negPDL1pos, a.count0PD1posPDL1neg, a.count0PD1posPDL1pos, a.count1PD1negPDL1neg, a.count1PD1negPDL1pos, a.count1PD1posPDL1neg, a.count1PD1posPDL1pos, a.count2PD1negPDL1neg, a.count2PD1negPDL1pos, a.count2PD1posPDL1neg, a.count2PD1posPDL1pos, a.count3PD1negPDL1neg, a.count3PD1negPDL1pos, a.count3PD1posPDL1neg, a.count3PD1posPDL1pos, a.count4PD1negPDL1neg, a.count4PD1negPDL1pos, a.count4PD1posPDL1neg, a.count4PD1posPDL1pos, a.count5PD1negPDL1neg, a.count5PD1negPDL1pos, a.count5PD1posPDL1neg, a.count5PD1posPDL1pos
from DiscriminantsTableDR_allcells d
left join AllNeighborsSummaryDR a on d.cellid = a.cellid and d.distancebinselectionid = a.distancebinselectionid
go